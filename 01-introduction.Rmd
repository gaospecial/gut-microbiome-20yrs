...
```{r trend-U1U2, fig.cap="文献自2013年以来的使用次数和近180天的使用次数对比"}
...
在关键字的排行榜中（图 \@ref(fig:summary-ID)）。
...
```{r summary-SC,fig.cap="SCI数据库中的研究方向"}
# SC, 研究方向
i <- 8
df <- tag_summary[[i]] %>% as.data.frame() %>%
  filter(Tab!="NA") %>%
...
文献耦合是指若文献A和文献B引证了相同的参考文献，则它们之间构成耦合关系，它们所包含相同参考文献的个数成为耦合强度。从文献耦合的概念上看，一个文献引用的参考文献越多，那么它将有越多的机会与其它文献建立耦合关系。为了消除这种影响，通常需要对原始数据使用 Jaccard 或 Salton 方法进行标准化处理，来计算相对的耦合强度。文献耦合测量的是文献间的静态关系，已经发表的论文的耦合强度不会随时间的变化而变化。
...
```{r eval=F}
histResults <- histNetwork(highly_cited,min.citations = 10,sep = ";")
net <- histPlot(histResults,size = 5)
```

### 共被引网络

共被引是指当两篇（或多篇）文献同时被后来一篇（或多篇）论文所引用的关系，同时引用这两篇论文的文献数目则称为共被引强度。文献的共被引关系会随着时间的变化而变化。通过对文献共被引网络的研究可以探究科学的发展和演进动态。
