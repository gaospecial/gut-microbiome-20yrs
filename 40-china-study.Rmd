...
我们使用历史引证网络分析的方法来厘清我国肠道菌群的研究脉络（图 \@ref(fig:china-histnet)）。历史引证网络中共包含了19篇关键文献，其主要信息列示在表 \@ref(fig:china-histnet-artilces) 中。接下来，我们将逐一回顾一下这些文献。
...
```{r}
keyword_network(Taiwan_study,2000,2019,field = "ID",nNode = 30)
```
...
```{r}
keyword_network(HK_study,2000,2019,field = "ID",nNode = 30)
```
...
```
```{r}
keyword_network(Macao_study,2000,2019,field = "ID",nNode = 30,edge_weight_cutoff = 2)
```
