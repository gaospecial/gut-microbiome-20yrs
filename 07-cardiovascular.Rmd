...
real_paper <- papers %>% 
  filter(status==3 & classify=="audit") %>%
...
```{r}
widge <- wordcloud2(chinese_keywords,fontFamily = "微软雅黑")
markdown_widget(widge,path="./_bookdown_files/bookdown_files/", filename = "chinese_wordcloud.png")
```
...
widge <- wordcloud2(english_keywords,fontFamily = "微软雅黑")
markdown_widget(widge,path="./_bookdown_files/bookdown_files/",filename = "eng_wordcloud.png")
```
...
在20年的数据中一共找到 `r nrow(M2)` 篇文献与“心血管”疾病相关，其中有 `r nrow(M2 %>% filter(HC==TRUE))` 篇高被引论文。
...
net <- networkPlot(net_matrix,n=80,type = "kamada",Title = "作者合作网络",halo=T,edges.min=3,size=T)
```
...
# TOP author的单位
df <- M2 %>% 
  filter(str_detect(C1,paste(top$author,collapse ="|")))
```
...
net <- networkPlot(net_matrix[selected,selected],n=30,Title = "关键词共现网络",type="kamada",size=T,halo=T,alpha=1)
```
...
```{r}
nonsense_keyword <- paste0(search_keywords,collapse = "|") %>% toupper()
```
