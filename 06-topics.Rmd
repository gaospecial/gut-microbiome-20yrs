...
```{r}
# Impact factor
file <- "./data/2019_Impact_factor.xlsx"
journal_IF <- openxlsx::read.xlsx(file, startRow=3) %>% 
  unique() %>%  # 一些行是重复的
  select(SO,impact_factor) %>%
  as_tibble() %>% 
  mutate_at("impact_factor",as.numeric) %>%
  mutate_at("SO",toupper) %>%
...
```{r}
# 读取近五年的文献资料
M <- read_rds("data/M_GUT_MICROBIOME.RDS") %>% as_tibble() %>%
  filter(PY>2014)
```
...
有些期刊已经没有影响因子，对应的文章有 `r sum(is.na(M$impact_factor))` 篇。具体如下(只显示前20行）：

```{r}
# 检查数据
M %>% filter(is.na(impact_factor)) %>% select(SO,impact_factor) %>% group_by(SO) %>%
  summarise(Records =n()) %>%
