...
knitr::opts_chunk$set(fig.width = 8,
                      ...
                      out.width = "100%",
                      fig.showtext = TRUE,
                      screenshot.opts = list(delay = 5, cliprect = 'viewport')
                      )
...
cran_packages <- c("tidyverse","lubridate","RColorBrewer","export","gganimate","ggraph")

github_packages <- c("gaospecial/wordcloud2","gaospecial/mrgut","gaospecial/ggVennDiagram","gaospecial/ccgraph")

success <- lapply(cran_packages, function(pkg) {
  if (system.file(package = pkg) == '') install.packages(pkg)
})

success <- lapply(github_packages, function(pkg) {
  name <- gsub(".*\\/","",pkg)
  if (system.file(package = name) == '') devtools::install_github(pkg)
})
...
```{r}
# 读取全部论文数据
M <- readRDS("data/M1.RDS") %>% filter(PY < 2020)

# 使用关键字、标题和摘要组成内容
M %<>% unite(content, ID,DE,TI,AB, sep=" ", remove = FALSE)

# 无影响因子期刊分组
M$group <- forcats::fct_explicit_na(M$group, na_level = "NA")
M$group <- relevel(M$group,"NA")  # 画图时会出现在最上面

# 核心文献 M core
M <- M %>% group_by(PY) %>% 
  mutate(CORE=ifelse(percent_rank(LCS)>=0.95,TRUE,FALSE)) %>% 
  ungroup()
MC <- M %>% filter(CORE==TRUE)
...
```{r}
authorProdOverTime <- mrgut::authorProdOverTime
...
```{r}
# eval types
only_debug <- FALSE             # 测试代码，提供额外信息
keep_but_not_include <- FALSE   # 有用的图片，暂时没有收录
under_construction <- FALSE     # 计划中的分析
run_and_save <- FALSE           # 耗时较长的运算过程，运行后保存，使用保存的结果而不用每次都运算
```
...
蓦然回首，特别是2008年美国国立卫生研究院（NIH）率先提出人体微生物组计划[^index-1]以来，“肠道菌群”已经兴起了十余年。而“肠道菌群”的研究岂止于此，2019年6月 Nature 杂志特别总结了“人类微生物组”研究的里程碑事件，将研究追溯到最早 1944 年的厌氧微生物分离培养 [@clarkCulturingAnaerobes2019]。
...
[^index-1]: 关于人类微生物组计划的一般常识，请参见[维基百科](https://en.wikipedia.org/wiki/Human_Microbiome_Project)，[百度百科](https://baike.baidu.com/item/%E4%BA%BA%E7%B1%BB%E5%BE%AE%E7%94%9F%E7%89%A9%E7%BB%84%E8%AE%A1%E5%88%92/10745945)。
...
本书的写作遵循**可重复研究**的范式，我们采用的主要研究手段是文献计量学方法，基于 **bibliometrix**\index{bibliometrix} [@R-bibliometrix] 和 R 语言数据科学工具包提供的方法 **tidyverse**\index{tidyverse} 和一系列可视化工具如 **DT**\index{DT} [@R-DT]、**VisNetwork**\index{visNetwork} [@R-visNetwork]、**wordcloud2**\index{wordcloud2} [@R-wordcloud2] 等。分析的数据来自于 Web of Science 数据库的核心合集（参见附录\@ref(data-source)）。最终，我用了两个 R 包编译这本书，分别是 **knitr**\index{knitr} [@xie2015] 和 **bookdown**\index{bookdown} [@R-bookdown]。
