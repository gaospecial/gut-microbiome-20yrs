...
在排除了我们使用的搜索限定词之后（参见 \@ref(data-source)），可以得出最受关注的关键字可以分为几类：首先，益生菌（PROBIOTICS）是最受关注的一个词语，还包括排在后面的PREBIOTICS，以及明星益生菌乳酸杆菌（LACTOBACILLUS）和双歧杆菌（BIFIDOBACTERIUM），这是第一类；接下来是炎症（INFLAMMATION）、肥胖（OBESITY）、失调（DYSBIOSIS）、克罗恩病（CORHN'S DISEASE）和溃疡性结肠炎（ULCERATIVE COLITIS）等一些疾病相关的关键字；其次，是短链脂肪酸、饮食、抗生素、营养、代谢等与肠道菌群的微生态密切相关的共识（图 \@ref(fig:summary-keyword)）。
...
```{r summary-keyword-wordcloud, fig.cap="关键字词云"}
df <-  rbind(df1,df2) %>% group_by(Tab) %>% summarise(n=sum(n)) %>% arrange(desc(n)) %>% head(100)
wordcloud2(df,size=0.4,fontFamily = "微软雅黑")
```


二者共同关注的内容在于几种常见的肠道相关疾病以及益生元等几个点，下面的词云展示了作者关键词和系统关键词合并之后最常见的200个关键词频次（图 \@ref(fig:summary-keyword-wordcloud)）。
...
top200_keyword <- table(keyword$keyword) %>% sort(decreasing = TRUE) %>% head(200)
top200_keyword_matrix <- keyword %>% 
  filter(keyword %in% names(top200_keyword)) %>%
  group_by(PY,keyword) %>%
  summarise(freq=n()) %>%
  pivot_wider(id_cols = "keyword",names_from = "PY",values_from = "freq",values_fill = list("freq"=0))
mat <- log10(as.matrix(top200_keyword_matrix[-1]) + 1)
